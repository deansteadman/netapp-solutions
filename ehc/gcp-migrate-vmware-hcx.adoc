...
= Migrate workloads to Google Cloud NetApp Volumes on Google Cloud VMware Engine using VMware HCX - Quickstart guide
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
One of the most common use cases for the Google Cloud VMware Engine and Google Cloud NetApp Volumes is the migration of VMware workloads. VMware HCX is a preferred option and provides various migration mechanisms to move on-premises virtual machines (VMs) and its data to Google Cloud NetApp Volumes NFS datastores.

Author(s): NetApp Solutions Engineering

== Overview: Migrating virtual machines with VMware HCX, Google Cloud NetApp Volumes, and Google Cloud VMware Engine (GCVE)
...
This document provides step-by-step guidance for provisioning Google Cloud NetApp Volumes followed by downloading, deploying, and configuring VMware HCX, including all its main components in on-premises and the Google Cloud VMware Engine side including Interconnect, Network Extension, and WAN optimization for enabling various VM migration mechanisms.

[NOTE]
VMware HCX works with any datastore type as the migration is at the VM level. Hence this document is applicable to existing NetApp customers and non-NetApp customers who are planning to deploy Google Cloud NetApp Volumes with Google Cloud VMware Engine for a cost-effective VMware cloud deployment.
...
* Google Cloud NetApp Volumes NFS volume should be mounted as a datastore in Google Cloud VMware Engine.  Follow the steps detailed in this https://cloud.google.com/vmware-engine/docs/vmware-ecosystem/howto-cloud-volumes-service-datastores[link^] to attach Google Cloud NetApp Volumes datastores to Google Cloud VMware Engines hosts.
=====
...
Whether you’re targeting all-cloud or hybrid cloud and data residing on any type/vendor storage in on-premises, Google Cloud NetApp Volumes and HCX provide excellent options to deploy and migrate the application workloads while reducing the TCO by making the data requirements seamless to the application layer. Whatever the use case, choose Google Cloud VMware Engine along with Google Cloud NetApp Volumes for rapid realization of cloud benefits, consistent infrastructure, and operations across on-premises and multiple clouds, bidirectional portability of workloads, and enterprise-grade capacity and performance. It is the same familiar process and procedures used to connect the storage and migrate VMs using VMware vSphere Replication, VMware vMotion, or even network file copy (NFC).

== Takeaways

The key points of this document include:

* You can now use Google Cloud NetApp Volumes as a datastore on Google Cloud VMware Engine SDDC.
* You can easily migrate data from on-premises to Google Cloud NetApp Volumes datastore.
* You can easily grow and shrink the Google Cloud NetApp Volumes datastore to meet the capacity and performance requirements during migration activity.
...
+
https://cloud.google.com/netapp/volumes/docs/discover/overview[https://cloud.google.com/netapp/volumes/docs/discover/overview^]
