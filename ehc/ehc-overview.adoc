...
By integrating with NetApp Cloud Volumes, such as Azure NetApp Files, Amazon FSx ONTAP, Google Cloud NetApp Volumes, and Cloud Volumes ONTAP (available in all three major hyperscalers) customers now have options to independently scale their storage separately, and only add compute nodes to the SDDC cluster as needed.
...
Azure NetApp Files, Amazon FSx ONTAP, Google Cloud NetApp Volumes, Cloud Volumes ONTAP (available in all three major hyperscalers) can be used in conjunction with guest VMs. This hybrid storage architecture consists of a vSAN datastore that holds the guest operating system and application binary data. The application data is attached to the VM through a guest-based iSCSI initiator or the NFS/SMB mounts that communicate directly with Amazon FSx ONTAP, Azure NetApp Files, Google Cloud NetApp Volumes and Cloud Volume ONTAP respectively. This configuration allows you to easily overcome challenges with storage capacity as with vSAN, the available free space depends on the slack space and storage policies used.
...
From a high-level perspective, this architecture (shown in the figure below) covers how to achieve hybrid Multicloud connectivity and app portability across multiple cloud providers using Azure NetApp Files, Google Cloud NetApp Volumes, and NetApp Cloud Volumes ONTAP as an additional in-guest storage option.
