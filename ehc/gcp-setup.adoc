...
Click on the “New Private Cloud” button and enter the desired configuration for the GCVE Private Cloud. On “Location”, make sure to deploy the private cloud in the same Region/Zone where Google Cloud NetApp Volumes or CVO is deployed, to ensure the best performance and lowest latency.
...
This will ensure that the VPC network where Google Cloud NetApp Volumes or Cloud Volumes ONTAP instances are running is able to communicate with the GCVE Private Cloud. For Google Cloud NetApp Volumes, establish a private service access connection between VMware Engine and NetApp Volumes. For detailed steps, follow this link:https://cloud.google.com/vmware-engine/docs/vmware-ecosystem/howto-cloud-volumes-datastores-gcve[link].
...
[%collapsible]
====
Refer link:https://cloud.google.com/vmware-engine/docs/vmware-ecosystem/howto-cloud-volumes-datastores-gcve[Procedure to deploy supplemental NFS datastore with Google Cloud NetApp Volumes to GCVE]
====
